...
class V:
    ...
    def for_vendetta(func):
        def f(*args, **kvargs):
            ...
            if func.__name__ in V.a:
                V.a[func.__name__][0] += 1
            else:
                V.a[func.__name__] = [0, "msg"]
...
class Path:
    ...
    def __init__(self, column=15, row=15, nb_item=3):
        ...
        Grid.dic = {x: 0 for x in ["start"] + list_item + ["final_goal"]}
        Grid.column = column
        Grid.row = row
        Grid.all = {
            Case((x + 1, y + 1)) for y in range(column) for x in range(row)
        }
        Grid.status = [None]
    ...
    def by_load_defaut_map(self):
        ...
        for i in range(1, 4):
            ...
            Grid.dic["item_" + str(i)] = random.sample(Grid.path - deja, 1)[0]
        else:
            Grid.object = {
                Grid.dic["item_" + str(i + 1)]
    ...
    def some_space():
        ...
        if grid_size < 225:
            return random.randrange(1, 101) >= 97
    ...
    def by_path_generator(self, failed=[0], all_loop=[0]):
        ...
        print("starting path generator main loop")
        loop = 0
        while not Grid.dic["final_goal"]:
            ...
            future_position = [
                i
                for i in self.near_position(*actual_position)
        ...
        for y in y_range:
            ...
            for x in x_range:
                ...
                if (x, y) == Hero.pos:
                    ...
                    symbol = "S"
                elif (x, y) in Grid.object:
                    symbol = "o"
                elif (x, y) == Grid.dic["final_goal"]:
                    symbol = "X"
                elif (x, y) in Grid.path:
                    symbol = " "
