...
class Grid:
	...
	def road(self):
		...
		start = (random.randrange(15),random.randrange(15))
		
		self.win_way.append(start)
		self.canvas.itemconfig( self.dict_grid[self.return_coord_string(start)].id, fill='red') 
		...
		while road_finish != True :
			...
			direction = random.choice((	# random x like this :
			( + 1 , + 0 ) ,				# 	 [X]
			( - 1 , + 0 ) ,				# [X][O][X]
			( + 0 , + 1 ) ,				#    [X]
